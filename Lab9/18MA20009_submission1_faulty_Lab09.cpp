...
bool is_loop_found_func(vpi next_cell_options, pi nbc){
...
void find_loop(stack<pi> &loop, vvi mat, vvi tableau, pi curr_cell, vvb &flag_mat, bool &is_loop_found, pi nbc, int &length_of_cycle){
	...
	if(is_loop_found_func(next_cell_options, nbc)){
		if(length_of_cycle > 2){
			cout<<"Found a loop of length > 2"<<endl;
			is_loop_found = 1;
			return;
		} 
	}
	...
	for(int i=0;i<next_cell_options.size();i++){
		...
		if((mat[next_cell.first][next_cell.second]!=-1) && (flag_mat[next_cell.first][next_cell.second] == 0)){
			loop.push(next_cell);
			...
			length_of_cycle++;
			find_loop(loop, mat, tableau, next_cell, flag_mat, is_loop_found, nbc, length_of_cycle);
			...
			//cout<<"Already visited/non allocated"<<endl;
		}
	}
}
...
void modify_mat(vvi &mat, vvi tableau, pi nbc, bool &is_loop_found){
	...
	find_loop(loop, mat, tableau, nbc, flag_mat, is_loop_found, nbc, length_of_cycle);
	if(!is_loop_found){
	...
	//Now that we have detected the loop, now update mat
	int min_till_now = INT_MAX;
	for(int i=0;i<loop_vec.size();i++){
		if(i%2){
			pi curr_cell = loop_vec[i];
