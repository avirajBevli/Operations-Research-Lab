...
vvd construct_bigM_tableau(vvd mat, vd objective_fn, vi slack_list, vi surplus_list, vi artificial_list){
	...
	//construct the first 2 columns of the tableau
	/*for(int i=0;i<num_rows;i++){
		tableau[i][0] = ;
		tableau[i][1] = num_cols-num_rows+i+1;//column index of the basis variable inside the tableau
	}*/
...
void solve_with_bigM(vvd tableau, vd objective_fn, bool is_max, vi artificial_list){
	int num_rows = tableau.size();
	int num_cols = tableau[0].size();
	...
	for(int i=0;i<num_rows;i++){
		if(is_in(tableau[i][1]-1, artificial_list)){
			cout<<"tableau "<<i<<":  "<<tableau[i][1]<<" is in artificial_list!"<<endl;
			cout<<"INFEASIBLE SOLUTION since even after termination the artificial variable is non zero...returning!!!"<<endl;
			return;
		}
	}
...
int main(){
	...
	if(is_problem_simplex){
		...
		solve_with_bigM(tableau, objective_fn, is_maximisation, artificial_variable_list);
	}	
}
