...
int find_alternate_basis(vd deviations, int num_rows){
...
void solve_with_simplex(vvd tableau, vd objective_fn){
	...
	while(1){
		...
		//cout<<"hi"<<endl;
		//cout<<"entering_var_col_index: "<<entering_var_col_index<<endl;
	...
	if(do_multiple_solutions_exist(deviations, tableau.size())){
		int entering_var_col_index = find_alternate_basis(deviations, tableau.size());
		int leaving_var_row_index=-1;
		double min_ratio = double_MAXX;
		for(int i=0;i<num_rows;i++){
			if(tableau[i][entering_var_col_index] <= 0) continue;
		...
		cout<<"Along with the mentioned point, an alternate point s.t. the optimisation function is optimised is: ";
		return;
	}
}
